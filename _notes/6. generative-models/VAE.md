...
>**방향성 확률 모델**(directed probabilistic models): 원인과 결과 관계 $$p(z) \rightarrow p(x\mid z)$$를 그래프로 나타내는 모델(directed graphical model)을 의미. 대표적으로 bayesian network가 있음.  
>**잠재 변수**(latent variable): 데이터 $$x$$를 생성하지만 직접 관찰할 수는 없는 변수임. 예를 들어 이미지($$x$$)를 생성하는 '얼굴 표정'이나 '글씨 스타일'($$z$$)같은 특징임.
>**사후 분포**(posterior distribution): 관찰된 데이터 $$x$$가 주어졌을 때, 이 데이터를 생성했을 잠재 변수 $$z$$의 분포 $$p(z\mid x)$$를 의미함.

저자는 대규모 데이터셋에 확장 가능하며, 약간의 미분 가능성 조건 하에서는 다루기 힘든 경우에도 작동하는 확률적 변분 추론 및 학습 알고리즘을 소개함.  
>**변분 추론**(variational inference): intractable한 사후 분포 $$p(z\mid x)$$를 $$q(z\mid x)$$라는 다루기 쉬운 근사 분포(예: 가우시안)로 대체하여 추론하는 기법  
...
>**변분 하한**: $$p(z\mid x)$$를 $$q(z\mid x)$$로 근사할 때, 두 분포가 얼마나 다른지(KL-divergence)를 측정하는데, 이 과정에서 유도되는 목적 함수($$\mathcal{L}(\theta,\phi;x^{(i)})$$)임.  VAE는 이 ELBO를 최대화하도록 학습함. 이때 ELBO는 $$q_\phi(z\mid x)$$에 대한 기댓값($$\mathbb{E}_{q_{\phi}(z\mid x)}[\dots]$$)을 포함함. 이 기댓값을 계산하기 위해 $$q_\phi$$로부터 $$z$$를 샘플링($$z \sim q_\phi(z\mid x)$$)해야 하는데, '샘플링' 연산은 미분이 불가능함. 따라서 q의 파라미터 $$\phi$$로 gradient가 역전파될 수 없기에 $$\phi$$를 학습시킬 수 없었음.  
>**재매개변수화**: q에서 z를 샘플링($$q_{\phi}(z\mid x)$$) 대신 $$z = g_\phi(\epsilon, x)$$ (예: $$z = \mu_\phi(x) + \sigma_\phi(x) \odot \epsilon$$, 여기서 $$\epsilon \sim \mathcal{N}(0, 1)$$) 처럼 $$z$$를 $$x$$와 파라미터($$\phi$$)에 대한 결정론적 함수 $$g_{\phi}$$와 외부 노이즈 $$\epsilon$$의 결합으로 바꿈. 이로써 z를 샘플링하는 대신 $$\epsilon$$을 샘플링하여 z를 $$\phi$$에 대해 미분 가능한 함수로 표현할 수 있음. 따라서 전체 목적 함수를 $$\phi$$에 대해 미분 가능하여 SGD로 최적화하는 것이 가능해짐.
...
>근사 추론 모델: VAE는 $$q_\phi(z\mid x)$$를 신경망(Encoder)으로 만듦. 이 신경망은 $$x$$를 입력받아 최적의 $$q$$ (예: $$q$$의 파라미터인 $$\mu$$와 $$\sigma$$)를 한 번의 계산(forward pass)으로 출력함. 이로써 개별 데이터마다 $$z$$를 최적화하는 대신 $$x$$를 $$z$$의 분포 파라미터로 매핑하는 추론 모델 자체를 학습시킴.이 모델의 파라미터 $$\phi$$는 모든 데이터에 대해 분담되어 공유됨. 따라서 학습이 끝난 후 새로운 $$x$$가 들어와도 즉시 $$z$$의 분포를 추론할 수 있어 효율적임.(인식모델은 $$x$$를 보고 $$z$$를 인식한다고 해서 붙여진 이름임.)

위 두가지 기여는 이후 실제 실험으로 보여짐.  

> **Q.** intractable한 $$p(z\mid x)$$와 $$q(z\mid x)$$의 차이는 무엇인가? $$q(z\mid x)$$를 전개했을 때 $$q(x)$$를 포함하는 intractable한 분포가 아닌가?  
> **A.** 실제 사후 분포 $$p(z\mid x)$$는 베이즈 정리에 의해 $$\frac{p(x\mid z)p(z)}{p(x)}$$로 표현되는데, $$p(x)$$(확률 변수 $$x$$에 대한 marginal likelihood)를 계산하는 것이 intractable함. $$p(x) = \int p(x\mid z)p(z)dz$$ 꼴의 적분이기 때문임. $$z$$는 연속 변수이고 $$p(x\mid z)$$는 복잡한 신경망(Decoder)이기 때문에, 모든 $$z$$에 대해 이 적분을 계산하는 것이 수학적으로 불가능함. 따라서 $$p(x)$$를 모르기 때문에 $$p(z\mid x)$$의 정확한 확률 밀도 값을 계산할 수 없음.  

따라서 $$p(z\mid x)$$를 베이즈 정리를 통해 $$p(x)$$를 계산하는 것을 포기하고, 이를 근사하기 위해 $$q_{\phi}(z\mid x)$$를 함수(신경망)로 직접 정의함.  

반면, $$q(z\mid x)$$는 우리가 선택한 근사 분포이므로 우리가 원하는 형태(예: 가우시안)로 정의할 수 있음. 따라서 $$q(z\mid x)$$는 다루기 쉬운 분포임.  

## 1 Introduction
앞서 언급한 난제에 대해 사후분포를 정확하게 계산하는것이 불가능하기에 $$p(z\mid x)$$와 비슷한 $$q(z\mid x)$$를 효율적으로 찾는 것을 목표로 삼음.  
...
> **A.** 먼저 해석적 해란 복잡한 방정식을 풀기 위해 논리적, 대수적 방법을 사용하여 정확한 형태의 해를 구하는 것을 의미. VB는 $$q(z)$$가 $$p(z\mid x)$$와 가장 비슷해지는 최적의 $$q(z)$$를 찾기 위해, 복잡한 기댓값 적분을 손으로 풀어야 했음. 하지만 이 적분이 복잡한 신경망일 경우에는 기댓값 적분이 복잡해져서 해석적으로 푸는 것이 불가능함. VAE는 이 해석적 해를 포기하고, Monte Carlo 샘플링을 사용하여 기댓값을 수치적으로 근사함으로써 이 문제를 해결함. 즉, 샘플 몇개 뽑아서 계산한 평균값으로 기댓값을 근사한다는 의미고 이게 SGVB임.
...
>3장에서 VAE의 $$q_\phi(z\mid x)$$를 대각 공분산(diagonal covariance)을 갖는 가우시안 $$\mathcal{N}(\mu, \sigma^2I)$$으로 가정하는데 공분산 행렬이 대각이라는 것은 모든 잠재 변수($$z_i$$, z의 모든 차원이)가 서로 독립이라고 가정하는 것과 같음. VAE는 이 전략(전통적인 방식)을 지적하기보다는 SGVB라는 새로운 최적화 방식을 제시함.
...
> **A.** 먼저 샘플링 과정이 왜 미분 불가능한지 다뤄보겠다. 미분(경사하강법)이 가능하려면, 파라미터($$\phi$$)의 작은 변화가 최종 손실(Loss)에 결정론적이고 연속적인 영향을 주어야 함. $$z$$는 $$q_{\phi}(z\mid x)$$에서 샘플링되는데 이 과정이 $$\phi$$에 대해 결정론적이지 않고 확률적이기 때문에 $$\phi$$에서 $$z$$로 이어지는 경로에 gradient가 흐를 수 없음. $$z$$는 $$\phi$$에 대한 함수가 아닌 샘플일 뿐임.  
...
$$
q_\phi(z\mid x) = Encoder Neural Network
$$  
...
<div style="text-align: center;" markdown="1">
  ![6_vae_figure_1.png]({{ site.baseurl }}/assets/6_vae_figure_1.png){: width="500" }
</div>

VAE가 다루는 확률모델의 구조를 도식화한 것.  
실선은 생성모델($$decoder$$) $$p_{\theta}(z)p_{\theta}(x\mid z)$$을 나타냄. $$z \rightarrow x$$방향의 화살표는 $$z$$가 원인이 되어 데이터 $$x$$를 생성한다는 의미.  
점선은 변분 근사(Variational approximation, $$encoder$$) $$q_{\phi}(z\mid x)$$를 나타냄. $$x \rightarrow z$$방향의 화살표는 관찰된 데이터 $$x$$로부터 잠재 변수 $$z$$를 추론한다는 의미.  
...
(2) 어떤 조건부 분포(conditional distribution) $$p_{\theta^{*}}(x\mid z)$$에서 값 $$x^{(i)}$$가 생성됨.    
이때 $$\theta^*$$는 알지 못하는 true 파라미터를 의미함.  

우리는 사전 분포 $$p_{\theta^{*}}(z)$$와 우도(likelihood) $$p_{\theta^{*}}(x\mid z)$$가 파라미터 분포족 $$p_{\theta}(z)$$와 $$p_{\theta}(x\mid z)$$에서 왔다고 가정하고, 이들의 확률 밀도 함수(PDF)가 $$\theta$$와 $$z$$ 모두에 대해 거의 어디서나(almost everywhere) 미분 가능하다고 가정함.  
...
> **A.** $$p_{\theta}(x\mid z)$$는 디코더가 출력하는 확률분포임. 잠재 변수 $$z$$가 주어졌을 때 데이터 $$x$$가 생성될 확률(밀도)임. 이때 부드럽게(Smoothly)변한다는 것은 수학적으로 미분 가능하다는 뜻임. 또, 이미지 도메인에서 고차원 내 두 점 사이 이미지가 두 이미지의 특성을 가지며 부드럽게 변함을 의미.

실제로 $$\theta^{*}$$와 $$z^{i}$$에 대해 알 수 없음.  

매우 중요하게도, marginal probabilities나 posterior probabilities에 대해 단순 가정을 하지 않음. 기존의 많은 모델은 계산을 쉽게 하기 위해 $$p_{\theta}(x)$$나 $$p_{\theta}(z\mid x)$$에 대해 특정한 형태(예: 정규분포)를 가정함. 하지만 VAE는 확률분포를 신경망으로 모델링하여 이러한 가정을 하지 않음으로 데이터가 매우 복잡한 분포를 따르거나, 잠재변수와 데이터 사이의 관계가 매우 복잡해도(비선형적이어도) 상관없다는 뜻임.  
...
- $$p(z\mid x)$$ 계산 불가 (베이지안 정리를 통해 도출된 분모 $$p(x)$$를 모름) -> EM 알고리즘 사용 불가능
- 평균 장 VB적분 계산 불가 -> 전통적인 VB(variational bayes) 사용 불가능

즉, 기존의 모든 표준적인 방법론이 막히는 상황임.  
이런 intractable한 상황은 매우 흔하며, 적당히 복잡한 우도함수 $$p_{\theta}(x\mid z)$$, 예를 들어 비선형 은닉층(nonlinear hidden layer)을 가진 신경망 같은 경우에 나타남.(신경망은 비선형 함수이기 때문에, 이를 포함한 적분식은 손으로 풀 수 없기 때문임.)  
...
- 생성모델(Decoder, $$p_{\theta}(x\mid z)$$)의 파라미터 $$\theta$$를 최대 우도 추정치(MLE) 또는 MAP 추정치로 학습하여 데이터를 생성하는 것.
- 인식 모델(Encoder, $$q_{\phi}(z\mid x)$$)의 파라미터 $$\phi$$를 학습하여 잠재 변수 $$z$$에 대한 근사 사후 분포를 추론하는 것.
- $$p_{\theta}(x)$$를 대략적으로라도 아는 것. 어떤 데이터 $$x$$가 '정상적인 데이터'인지 '이상한 데이터'인지를 판단하는 능력.

위 문제를 해결하기 위해, 실제 사후 분포 $$p_{\theta}(z\mid x)$$에 대한 근사(approximation)인 인식 모델(encoder)$$q_{\phi}(z\mid x)$$를 도입함.  
...
따라서 본 논문에서는 인식 모델 $$q_{\phi}(z\mid x)$$를 **확률적 인코더**(probabilistic encoder)라고 부를것임. 데이터 포인트 x가 주어졌을 때, 이 모델은 데이터 $$x$$를 생성했을 가능성이 있는 코드 값 **z에 대한 분포를 산출**하기 때문.(일반적인 오토인코더는 고정된 $$z$$벡터하나(deterministic)만 출력하지만 VAE는 $$z$$의 분포(평균과 분산)를 출력.)    
비슷한 맥락에서 생성 모델 $$p_{\theta}(x\mid z)$$를 **확률적 디코더**(probabilistic decoder)라고 부름. 코드 $$z$$가 주어졌을 때, 이 모델은 가능한 대응 값 $$x$$에 대한 **분포를 산출**함.($$z$$를 받아 $$x$$를 바로 뱉는게 아닌(결정론적X) $$x$$가 나올 확률 분포(베르누이 분포의 $$p$$값, 가우시안의 평균과 분산)를 출력하기 때문에 디코더가 정의한 확률 분포 함수 자체를 의미)  
...
$$
\log p_{\theta}(x^{(i)})\ge\mathcal{L}(\theta,\phi;x^{(i)})=\mathbb{E}_{q_{\phi}(z\mid x)}[-\log q_{\phi}(z\mid x)+\log p_{\theta}(x,z)]
$$  

$\mathbb{E}_{q_{\phi}(z\mid x)}[-\log q_{\phi}(z\mid x)+\log p_{\theta}(x,z)]$에서 $\log p_\theta(x,z)$를 $\log p_\theta(x^{(i)}\mid z)p_\theta(z)$로 쪼개지므로, 위 $\mathcal{L}$를 다시 정리하면 VAE의 손실함수 형태가 됨.  

$$
\mathcal{L}(\theta,\phi;x^{(i)})=-D_{KL}(q_{\phi}(z\mid x^{(i)})\mid \mid p_{\theta}(z))+\mathbb{E}_{q_{\phi}(z\mid x^{(i)})}[\log p_{\theta}(x^{(i)}\mid z)]
$$  
...
> **Q.** $$\log p_{\theta}(x)$$에서 KL발산 항은 왜 등장하며, 이 KL발산 항을 삭제해도 되도 문제 없는가? 여기서 말하는 KL발산은 ELBO항 내 KL발산이 아닌 $$\log p_{\theta}(x)$$의 $$D_{KL}(q_{\phi}(z\mid x) \mid \mid  p_{\theta}(z\mid x))$$항임. 의미를 보면 추정한 가짜 사후 분포($$q$$)와 진짜 사후 분포($$p_{true}$$)사이의 차이를 나타냄.  
> **A.** 최적화 관점에서 목적함수를 들여다 보면 KL발산 항의 $$p_{\theta}(z\mid x)$$은 intractable하기 때문에 계산할 수 없음.
>따라서 이 항을 직접적으로 다룰 수 없음. 목표를 "우도(log p(x))를 직접 최대화하는 것" 대신 "우도의 하한선(ELBO)을 최대한 끌어올리는 것"으로 바꿈.

> **Q.** $$p_{\theta}(z\mid x)$$은 신경망이기 때문에 직접 다룰 수 있지 않은가?
> A. $$\theta$$가 붙어있다 = 신경망이 계산한다 라고 생각할 수 있지만 여기서 의미는 다름. $$p_{\theta}(z\mid x)$$는 신경망 $$\theta$$가 '출력하는 값'이 아니라, 신경망 $$\theta$$에 의해 수학적으로 '결정되는(implied) 값'인데, 계산이 불가능함.
> 	1. $$\theta$$는 디코더$$p_\theta(x\mid z)$$의 파라미터임.
> 	2. $$p_\theta(z\mid x)$$은 디코더의 역방향임. 이걸 구하려면 베이즈 정리를 써야함.
>
...
> 	3. 분자 $$p_\theta(x\mid z)$$는 디코더, $$p_\theta(z)$$는 가정한 $$\mathcal{N}(0, I)$$임으로 둘다 계산 가능함.
> 	4. 분모를 계산하려면 가능한 모든 z에 대해 확률$$p_\theta(x) = \int p_\theta(x\mid z)p_\theta(z) dz$$을 다 더해야(적분해야)함. 이 복잡한 비선형 신경망(디코더)를 포함한 수식을 모든 실수 공간 전체의 $$z$$에 대해 적분하는 것은 수학적으로 불가능(intractable)함.
> 따라서 이 값을 직접 계산하는 대신, 별도의 신경망(인코더)을 만들어 $$q_\phi(z\mid x)$$로 근사하는 것임.
...
$$\theta$$는 ELBO안쪽 $$\log p_{\theta}$$만 미분하면 됨(Monte Carlo 샘플링으로 해결가능). 그러나 $$\phi$$에 대한 gradient를 계산하는 것은 쉽지 않음. 왜냐하면 $$\mathbb{E}_{q_{\phi}(z\mid x)}$$에 대한 기댓값으로 표현되기 때문임.  
...
현재는 $$q_{\phi}(z\mid x)$$형태의 근사 사후 분포를 가정하지만, $$q_{\phi}(z)$$인 $$x$$에 조건부이지 않은 경우(인코더가 없는 경우)에도 적용될 수 있음.  

2.4절에 기술된 조건 하에 $$q_{\phi}(z\mid x)$$에서 z를 샘플링하는 과정을 $$\tilde{z}\sim q_{\phi}(z\mid x)$$를 (보조) 노이즈 변수 $$\epsilon$$의 미분 가능한 변환 $$g_{\phi}(\epsilon,x)$$를 사용하여 재매개변수화(reparameterize)할 수 있음.  
...
구체적인 방법은 2.4절에서 다룸.  
$$\mathbb{E}_{q_{\phi}(z\mid x^{(i)})}[f(z)]=\mathbb{E}_{p(\epsilon)}[f(g_{\phi}(\epsilon,x^{(i)}))]\simeq\frac{1}{L}\sum_{l=1}^{L}f(g_{\phi}(\epsilon^{(l)},x^{(i)}))$$ where $$\epsilon^{(l)}\sim p(\epsilon)$$  
...
$$
\tilde{\mathcal{L}}^{A}(\theta,\phi;x^{(i)})=\frac{1}{L}\sum_{l=1}^{L}\log p_{\theta}(x^{(i)}\mid z^{(i,l)}) + \log q_\phi(z^{(i,l)})-\log q_{\phi}(z^{(i,l)}\mid x^{(i)})
$$  
...
> KL 발산은 기본적으로 적분임. $$D_{KL}(q\mid \mid p) = \int q(z) (\log q(z) - \log p(z)) dz$$
...
<div style="text-align: center;" markdown="1">
  ![6_vae_algorithm.png]({{ site.baseurl }}/assets/6_vae_algorithm.png){: width="500" }
</div>
...
ELBO의 KL발산 항은 근사사후분포$$q_\phi(z)$$와 사전분포$$p(z)$$ 사이의 **규제**(인코더가 데이터x를 잠재공간으로 보낼 때, 그 분포가 정규분포 모양을 갖추도록 강제)역할이며, 다른 항$$\text{log p}_{\theta}(x^{(i)}\mid z)$$은 $$z$$에서 $$x$$로 복원했을 때 확률 밀도를 최대화함.(음의 재구성 오차: $$z$$를 통해 예측된 $$\tilde x$$와 실제$$x$$의 차이, $$MSE$$)  

인코더와 재매개변수화 트릭이 $$x \to z$$ 매핑(인코딩)과정을 담당함.  
$$z^{(i,l)}=g_{\phi}(\epsilon^{(l)},x^{(i)})$$ 여기서 $$z^{(i,l)}\sim q_{\phi}(z\mid x^{(i)})$$  

그 후 샘플 $$z^{(i,l)}$$는 $$\log p_{\theta}(x^{(i)}\mid z^{(i,l)})$$에 입력되는데 이는 $$z^{(i,l)}$$가 주어졌을 때 데이터 $$x^{(i)}$$의 확률 밀도(또는 질량)와 같음.(생성모델이(디코더) $$z \to x$$ 매핑 과정을 담당.)  
...
연속 확률 잠재 변수 $$z$$를 조건부 분포 $$z\sim q_\phi(x\mid z)$$에서 직접 샘플링하는 대신, 미분이 가능하도록 우회하는 대안을 사용.  
...
결정론적 매핑 $$z=g_{\phi}(\epsilon,x)$$가 주어졌을 때, 우리는 $$q_{\phi}(z\mid x)\prod_{i}dz_{i} = p(\epsilon)\prod_{i}d\epsilon_{i}$$임.  
>Q. "이게 도대체 무슨 말임?"
>A.

$$z$$공간에서의 확률 질량$$q(z)dz$$ 과 매핑된 $$\epsilon$$공간에서의 확률 질량 $$p(\epsilon)d\epsilon$$은 보존되어야 한다는 원리임.  
따라서, $$\int q_{\phi}(z\mid x)f(z)dz=\int p(\epsilon)f(z)d\epsilon=\int p(\epsilon)f(g_{\phi}(\epsilon,x))d\epsilon$$임.  
...
예를 들어 일변량 가우시안의 경우($$z\sim p(z\mid x)=\mathcal{N}(\mu,\sigma^{2})$$)에서 재매개변수화는 $$z=\mu+\sigma\epsilon$$이며, 여기서 $$\epsilon$$은 보조 노이즈 변수$$\epsilon\sim\mathcal{N}(0,1)$$임.  
가우시안 말고 또 어떤 분포에 이 트릭을 쓸 수 있는지 확장성을 설명함.  
	1. 계산 가능한 역 CDF(누적 분포 함수). 이 경우$$\epsilon\sim\mathcal{U}(0,I)$$(0과 1 사이의 균등 분포)로 두고, $$g_{\phi}(\epsilon,x)$$를 $$q_{\phi}(z\mid x)$$의 역 CDF로 설정함. CDF의 역함수만 알면, 0~1사이의 난수($$\epsilon$$)를 꽂아 넣어 원하는 분포의 샘플 $$z$$를 만들 수 있음. Examples: Exponential, Cauchy, Logistic, Rayleigh, Pareto, Weibull, Reciprocal, Gompertz, Gumbel and Erlang distributions.  
...
디코더 $$p_{\theta}(x\mid z)$$는 $$z$$를 입력받아 $$x$$의 분포 파라미터를 뱉는 MLP임. 이때 데이터($$x$$) 타입에 따라 MSE(가우시안)을 쓸지, BCE(베르누이)를 쓸지 결정됨.  
이때 디코더가 복잡한 비선형 신경망이므로, 역방향인 **실제 사후 분포** $$p_{\theta}(z\mid x)$$는 계산 불가능함.(베이즈 정리를 따라가보면 됨.)  
인코더 $$q_{\phi}(z\mid x)$$의 형태에는 많은 가정을 할 수 있지만, 실제(다루기 힘든) 사후 분포가 대략적으로 **대각 공분산**을 갖는 **가우시안** 형태를 띤다고 가정할 것이고, 이 경우에 변분 근사 사후분포($$q$$)를 대각 공분산 구조를 가진 다변량 가우시안으로 설정할 수 있음.  
...
이때 사전 분포와 근사 사후 분포(인코더: $$q_{\phi}(z\mid x)$$)는 모두 가우시안이기 대문에 KL 발산이 추정(샘플링) 없이 계산되고 미분될 수 있는 추정치 B를 사용하여 닫힌 형태(해석적 해)를 구할 수 있음.  

이 모델과 데이터 포인트 $$x^{(i)}$$에 대한 결과적인 추정치(Loss function)은 다음과 같음.  

$$
\mathcal{L}(\theta,\phi;x^{(i)})\simeq\frac{1}{2}\sum_{j=1}^{J}(1+\log((\sigma_{j}^{(i)})^{2})-(\mu_{j}^{(i)})^{2}-(\sigma_{j}^{(i)})^{2})+\frac{1}{L}\sum_{l=1}^{L}\log p_{\theta}(x^{(i)}\mid z^{(i,l)})
$$  
...
디코더 $$\log p_{\theta}(x^{(i)}\mid z^{(i,l)})$$는 모델링하는 데이터 타입(실수, 이진)에 따라 가우시안 또는 베르누이 MLP임.  
...
[Row98]에서는 PCA가 사전 분포 $$p(z)=\mathcal{N}(0,I)$$와 조건부 분포 $$p(x\mid z)=\mathcal{N}(x;Wz,\epsilon I)$$를 갖는 선형-가우시안 모델의 특수한 경우, 구체적으로는 $$\epsilon$$이 무한히 작은(infinitesimally small) 경우에 확률적인 성격이 사라지고 결정론적인 일반 PCA가 됨으로써 최대 우도(ML) 해에 해당한다는 것이 보여졌습니다. VAE는 이 PCA의 아이디어를 비선형(신경망)으로 확장하고, 노이즈($$\epsilon$$)를 0으로 없애지 않고 확률적 성질을 살려둔 일반화된 버전이라고 볼 수 있음.   
> Q. 이 상황에서 최대 우도 해란?
...
> 최대 우도 해(ML Solution): 이 확률 모델에서 관측 데이터 X가 등장할 확률($$likelihood, p(X\mid W, \sigma^2)$$)을 최대화하는 파라미터 $$W$$를 찾는 것을 의미.
...
>MAP기준:
>	일반적인 최대 우도(Maximum Likelihood)는 데이터$$p(X\mid \theta)$$만 최대화함.
>	MAP는 파라미터 자체의 확률인 사전 분포 $$p(\theta)$$까지 고려하여 $$p(\theta\mid X) \propto p(X\mid \theta)p(\theta)$$를 최대화함.
...
figure 4: 2차원 잠재공간 $$Z$$과 그에 대응하는 관측 공간 $$X$$를 시각화함. 잠재 공간의 사전 분포가 가우시안이므로, 단위 정사각형 위의 선형적으로 간격을 둔 좌표들을 가우시간의 역 CDF(누적 분포 함수)를 통해 변환하여 잠재 변수 $$z$$의 값들을 생성함. 그냥 일정한 간격으로 자르는게 아닌, 백분위수를 활용하여 가우시안 분포의 중앙은 촘촘하게, 양 끝 부분은 넓게 좌표($$z$$)가 잡힘. 이로써 데이터가 존재하는 영역을 더 균일하고 밀도 있게 시각화해줌. 이러한 각 $$z$$값에 대해, 학습된 파라미터 $$\theta$$를 사용(디코더를 통해 매핑)하여 대응하는 생성 확률 $$p_\theta(x\mid z)$$(의 평균값, 분산 고려X)를 플롯함. 결과로 부드러운 변화가 나옴으로써 VAE가 단순히 이미지를 외운 것이 아닌, 데이터가 존재하는 연속적인 공간의 구조를 학습했다는 증거임.  
figure 5: 다양한 차원 수의 잠재 공간에 대해, 학습된 mnist 생성 모델로부터 추출한 무작위 샘플들. VAE의 잠재 공간의 차원이 높아져도 과적합 없이 안정적으로 학습되며, 더 많은 정보를 표현할 수 있음을 보임.  

### Appendix B. Solution of $$-D_{KL}(q_{\phi}(z)\mid \mid p_{\theta}(z))$$, Gaussian case
변분 하한의 KL 발산 항은 종종 해석적으로 적분될 수 있는데, 이때 '해석적으로 적분된다'는 말은 복잡한 적분 기호를 풀어서 깔끔한 덧셈/뺄셈 공식으로 바꿀 수 있다는 뜻임.  
이때 전제 조건은 사전 분포 $$p(z)$$는 표준 정규분포 $$\mathcal{N}(0,I)$$와 사후 근사 $$q_\phi(z\mid x^{(i)})$$가 모두 가우시안인 경우의 해를 제시.  
...
$$-D_{KL}((q_{\phi}(z)\mid \mid p_{\theta}(z))=\int q_{\theta}(z)(\log p_{\theta}(z)-\log q_{\theta}(z))dz = \frac{1}{2}\sum_{j=1}^{J}(1+\log((\sigma_{j})^{2})-(\mu_{j})^{2}-(\sigma_{j})^{2})$$  
...
이 경우 $$p_{\theta}(x\mid z)$$를 다변량 베르누이 분포로 둠. 그 확률들은 단일 은닉층을 가진 완전 연결 신경망을 통해 $$z$$로부터 계산됨.  
$$\log p(x\mid z)=\sum_{i=1}^{D}x_{i}\log y_{i}+(1-x_{i})\cdot \log(1-y_{i})$$  
...
$$\log p(x\mid z)=\log\mathcal{N}(x;\mu,\sigma^{2}I)$$ where $$\mu=W_{4}h+b_{4}$$, $$\log\sigma^{2}=W_{5}h+b_{5}$$  
...
인코더 구조도 위와 똑같음.  
입력이 x, 출력이 z, 가중치와 편향은 변분 파라미터 $$\phi$$가 됨. $$\log q_\phi(z\mid x)$$  
...
$$p_{\theta}(x,z)$$를 우리가 샘플링하고 있는 생성 모델이라고 하고, 주어진 데이터 포인트 $$x^{(i)}$$에 대해 주변 우도 $$p_{\theta}(x^{(i)})$$를 추정하고자 함. $$p_{\theta}(x,z)=p_{\theta}(z)p_{\theta}(x\mid z)$$. 이 추정 과정은 세 단계로 구성됨.  
1단계: 사후 분포에서 샘플링.  
	진짜 사후 분포 $$\log p_{\theta}(z\mid x)$$는 모르지만 그 기울기$$\nabla_{z}\log p_{\theta}(z\mid x)$$는 우도와 사전 분포의 기울기 합: $$\nabla_{z}\log p_{\theta}(z\mid x)=\nabla_{z}\log p_{\theta}(z)+\nabla_{z}\log p_{\theta}(x\mid z)$$  
...
$$p_{\theta}(x^{(i)})\simeq(\frac{1}{L}\sum_{l=1}^{L}\frac{q(z^{(l)})}{p_{\theta}(z)p_{\theta}(x^{(i)}\mid z^{(l)})})^{-1}$$ where $$z^{(l)}\sim p_{\theta}(z\mid x^{(i)})$$  
...
앞쪽 괄호 $$\frac{p_{\theta}(x^{(i)}, z)}{p_{\theta}(x^{(i)})}$$는 베이즈 정리에 의해 사후 분포 $$p_{\theta}(z\mid x^{(i)})$$와 같음.  

$$
= \int p_{\theta}(z\mid x^{(i)}) \cdot \left( \frac{q(z)}{p_{\theta}(x^{(i)}, z)} \right) dz
$$  

적분의 정의: 어떤 확률 분포 $$p$$를 곱해서 적분한다는 것은 "그 분포 하에서의 평균(기댓값)을 구한다"는 뜻임. $$\int p(z) \cdot f(z) dz = \mathbb{E}_{p(z)}[f(z)]$$  
적분의 정의에 따라 위 유도 수식이 진짜 사후 분포 $$p_{\theta}(z\mid x^{(i)})$$에서의 기댓값으로 표현됨.  

$$
= \mathbb{E}_{z \sim p_{\theta}(z\mid x^{(i)})} \left[ \frac{q(z)}{p_{\theta}(x^{(i)}, z)} \right]
$$  
...
이때 $$p_{\theta}(x, z) = p_{\theta}(x\mid z)p_{\theta}(z)$$이므로 분모는 계산 가능함.  
위 수식의 의미가 진짜 사후 분포$$p(z\mid x)$$에서 뽑은 샘플 $$z^{(l)} \sim p_{\theta}(z\mid x^{(i)})$$들 가지고, $$\frac{q(z)}{p(x,z)}$$라는 비율을 계산해서 평균을 내면 그게 목표였던 $$\frac{1}{p(x)}$$임.  
부록 D 요약:  
- 구하려는 값의 역수($$1/p$$)를 적분식으로 씀.
- 분자 분모에 $$p(x,z)$$를 곱해서 베이즈 정리($$p(z\mid x)$$) 모양을 만듦.
- 그러면 식이 "$$p(z\mid x)$$에서 뽑은 샘플들로 $$\frac{q}{p(x,z)}$$의 평균을 구하라"는 뜻이 됨.
- 평균을 구한 뒤, 다시 역수를 취해서 원래 값($$p$$)을 찾음.

### Appendix E. Monte Carlo EM
몬테 카를로 EM알고리즘은 인코더를 사용하지 않음. 매번 데이터 x가 주어질 때마다 z공간을 탐색하며 어디가 확률이 높은 z인가를 찾아야 함. 대신 사후 분포의 기울기$$\nabla_{z}\log p_{\theta}(z\mid x)$$를 사전 분포의 기울기와 우도의 기울기의 합 $$=\nabla_{z}\log p_{\theta}(z)+\nabla_{z}\log p_{\theta}(x\mid z)$$ 으로 계산할 수 있음.  
...
이때 주변 우도를 정의하면 $$\log p_{\alpha}(X)=D_{KL}(q_{\phi}(\theta)\mid \mid p_{\alpha}(\theta\mid X))+\mathcal{L}(\phi;X)$$임.  
...
우변의 첫번째 항은 근사 사후 분포($$q_\phi(\theta)$$)와 실제 사후 분포($$p_\alpha(\theta\mid X)$$, intractable) 사이의 KL 발산을 나타내며, $$\mathcal{L}(\phi;X)$$는 주변 우도에 대한 변분 하한을 나타냄.(KL 발산은 non-negative이므로 $$\mathcal{L}(\phi;X)$$가 주변 우도에 대한 하한이 됨. 근사 사후 분포와 실제 사후 분포가 정확히 일치할 때 이 하한은 실제 주변 우도와 같아짐.)  
...
$$\mathcal{L}(\theta,\phi;x^{(i)})=\int q_{\phi}(z\mid x)(\log p_{\theta}(x^{(i)}\mid z)+\log p_{\theta}(z)-\log q_{\phi}(z\mid x))dz$$  
$$\mathcal{L}(\phi;X)$$와 $$\mathcal{L}(\theta,\phi;x^{(i)})$$의 우변에 대한 기대치는 분명 세개의 개별 기대값의 합으로 작성될 수 있으며, 두 번째와 세 번째 구성 요소는 때때로 해석적 해를 구할 수 있음. 예를 들어 $$p_\theta(x), q_\phi(z\mid x)$$가 모두 가우시안일 때에 해당하는데, 일반적으로 이러한 기대가 각각 다루기 어렵다고 가정함.  
재매개변수화 트릭의 이중 적용  
$$z$$뿐만 아니라 $$\theta$$에도 재매개변수화 트릭을 씀.  

---
Standard VAE는 $$z$$를 확률 변수로 취급하여 $$q_\phi(z\mid x)$$를 추론하지만, 신경망의 weight $$\theta$$는 고정된 값을 찾는 최적화 대상임. Full VB에서는 $$\theta$$도 불확실성을 가진 확률 변수로 보고, 이에 대한 사후 분포 $$p(\theta\mid X)$$를 근사하는 $$q_\phi(\theta)$$를 학습함. 이를 통해 모델 파라미터 자체의 불확실성을 모델링할 수 있음. 이 Full VB를 위한 추정량을 유도함.  
...
$$\log p_{\alpha}(X) = D_{KL}(q_{\phi}(\theta)\mid \mid p_{\alpha}(\theta\mid X)) + \mathcal{L}(\phi;X)$$  
	좌변: 데이터의 로그 우도.(Marginal Log Likelihood)  
	우변 첫째 항: 근사 사후 분포 $$q_\phi(\theta)$$와 실제 사후 분포 $$p_\alpha(\theta\mid X)$$간의 KL divergence(항상 0보다 큼.)  
...
>$$\log p(x) = \underbrace{\mathcal{L}(\phi; x)}_{\text{ELBO}} + \underbrace{D_{KL}(q_{\phi}(z\mid x) \mid \mid  p(z\mid x))}_{\text{KL Divergence}}$$
...
본문과 동일하게 두 근사 사후 분포 $$q_{\phi}(\theta)$$, $$q_{\phi}(z\mid x)$$에 대해 $$\tilde{z}\sim q_{\phi}(z\mid x)$$를 보조 노이즈 변수 $$\epsilon$$을 사용하여 $$\tilde{z}=g_{\phi}(\epsilon,x)$$로 재매개변수화할 수 있음  
이때 $$z$$에 대한 기댓값을 $$\epsilon$$에 대한 기댓값으로 바꿀 수 있음.  
$$\mathcal{L}(\theta,\phi;x^{(i)})=\int q_{\phi}(z\mid x)(\log p_{\theta}(x^{(i)}\mid z)+\log p_{\theta}(z)-\log q_{\phi}(z\mid x))dz$$  
$$= \int p(\epsilon)(\log p_\theta(x^{(i)}\mid z) + \log p_\theta(z) - \log q_\phi(z\mid x))\mid _{z=g_\phi(\epsilon,x^{(i)})}d\epsilon$$  
...
$$\mathcal{L}(\phi;X) = \int p(\zeta)(\log p_{\theta}(X)+\log p_{\alpha}(\theta)-\log q_{\phi}(\theta))\mid _{\theta=h_{\phi}(\zeta)}d\zeta$$  
...
$$f_{\phi}(x,z,\theta)=N\cdot(\log p_{\theta}(x\mid z)+\log p_{\theta}(z)-\log q_{\phi}(z\mid x))+\log p_{\alpha}(\theta)-\log q_{\phi}(\theta)$$  
...
$$\log q_{\phi}(z\mid x)=\log\mathcal{N}(z;\mu_{z},\sigma_{z}^{2}I)$$  
...
<div style="text-align: center;" markdown="1">
  ![6_vae_algorithm2.png]({{ site.baseurl }}/assets/6_vae_algorithm2.png){: width="500" }
</div>
...
$$q_{\phi}(z\mid x)$$ as $$\tilde{z}=\mu_{z}+\sigma_{z}\odot\epsilon$$, where $$\epsilon \sim \mathcal{N} (0, I)$$  
...
$$
\mathcal{L} \simeq \frac{1}{L} \sum_{l=1}^L \left( N \cdot \left( \text{KL}(z \text{ terms}) + \log p(x\mid z) \right) + \text{KL}(\theta \text{ terms}) \right)
$$  
...
근사 사후 분포 $$q_\phi(z\mid x)$$와 실제 사후 분포 $$p_\theta(z\mid x)$$ 사이의 KL 발산(차이)을 정의.  

$$
D_{KL}(q_\phi(z\mid x) \mid \mid  p_\theta(z\mid x)) = \mathbb{E}_{q_\phi(z\mid x)} \left[ \log \frac{q_\phi(z\mid x)}{p_\theta(z\mid x)} \right]
$$  

$$
= \mathbb{E}_{q_\phi(z\mid x)} [\log q_\phi(z\mid x) - \log p_\theta(z\mid x)]
$$  

2. 베이즈 정리(Bayes' Theorem) 적용  
여기서 실제 사후 분포 $$p_\theta(z\mid x)$$는 베이즈 정리에 의해 다음과 같이 표현됨.  
...
$$
D_{KL}(q_\phi(z\mid x) \mid \mid  p_\theta(z\mid x)) = \mathbb{E}_{q_\phi(z\mid x)} [\log q_\phi(z\mid x) - (\log p_\theta(x\mid z) + \log p_\theta(z) - \log p_\theta(x))]
$$  

괄호를 풀고 항을 정리.  

$$
= \mathbb{E}_{q_\phi(z\mid x)} [\log q_\phi(z\mid x) - \log p_\theta(x\mid z) - \log p_\theta(z) + \log p_\theta(x)]
$$  

4. $$\log p(x)$$의 분리 (상수 취급)  
여기서 $$\log p_\theta(x)$$는 $$z$$에 의존하지 않는 값(상수)이므로, 기댓값 $$\mathbb{E}_{q_\phi(z\mid x)}$$ 밖으로 나올 수 있음. (확률 분포 $$q(z\mid x)$$의 총합은 1이므로)  

$$
D_{KL}(q_\phi(z\mid x) \mid \mid  p_\theta(z\mid x)) = \underbrace{\mathbb{E}_{q_\phi(z\mid x)} [\log q_\phi(z\mid x) - \log p_\theta(x\mid z) - \log p_\theta(z)]}_{-\text{ELBO}} + \log p_\theta(x)
$$  
...
$$
\log p_\theta(x) = D_{KL}(q_\phi(z\mid x) \mid \mid  p_\theta(z\mid x)) - \mathbb{E}_{q_\phi(z\mid x)} [\log q_\phi(z\mid x) - \log p_\theta(x\mid z) - \log p_\theta(z)]
$$  
...
$$
\mathcal{L}(\theta, \phi; x) = \mathbb{E}_{q_\phi(z\mid x)} [\log p_\theta(x\mid z) + \log p_\theta(z) - \log q_\phi(z\mid x)]
$$  

$$
= \mathbb{E}_{q_\phi(z\mid x)} [\log p_\theta(x\mid z)] -\mathbb{E}_{q_\phi(z\mid x)} [\log q_\phi(z\mid x) - \log p_\theta(z)]
$$  

이 식을 다시 **재구성 오차(Reconstruction)** 와 **정규화(Regularization)** 항으로 보기 좋게 정리하면 최종적인 ELBO 식이 도출됨.  

$$
\mathcal{L}(\theta, \phi; x) = \mathbb{E}_{q_\phi(z\mid x)} [\log p_\theta(x\mid z)] - D_{KL}(q_\phi(z\mid x) \mid \mid  p_\theta(z))
$$  
...
### Proof 2. $$p(x\mid z)$$와 MSE(음의 재구성오차)가 수학적으로 동치인 이유
**Reconstruction Term**($$\log p_\theta(x\mid z)$$)이 **MSE(Mean Squared Error)** 와 동치가 되는 과정을 수학적으로 유도.  
핵심 전제는 **"디코더 $$p_\theta(x\mid z)$$가 다변량 정규분포(Multivariate Gaussian Distribution)를 따른다고 가정할 때"** 임.  
#### **수식 유도 과정**
...
$$
p(x) = \frac{1}{(2\pi)^{D/2} \mid \Sigma\mid ^{1/2}} \exp \left( -\frac{1}{2} (x - \mu)^T \Sigma^{-1} (x - \mu) \right)
$$  

우리의 가정($$\mu = \mu_\theta(z)$$, $$\Sigma = \sigma^2 I$$)을 대입.  

$$
p_\theta(x\mid z) = \frac{1}{(2\pi)^{D/2} (\sigma^2)^{D/2}} \exp \left( -\frac{1}{2\sigma^2} \mid \mid x - \mu_\theta(z)\mid \mid ^2 \right)
$$  

2. 로그 우도 (Log-Likelihood) 취하기: 우리가 최대화하려는 Reconstruction Term은 위 식에 로그($$\log$$)를 취한 것임.  

$$
\log p_\theta(x\mid z) = \log \left( \frac{1}{(2\pi \sigma^2)^{D/2}} \right) + \log \left( \exp \left( -\frac{1}{2\sigma^2} \mid \mid x - \mu_\theta(z)\mid \mid ^2 \right) \right)
$$  

로그의 성질($$\log(ab) = \log a + \log b$$, $$\log(e^x) = x$$)을 이용해 정리.  

$$
\log p_\theta(x\mid z) = \underbrace{-\frac{D}{2} \log(2\pi \sigma^2)}_{\text{Constant}} - \frac{1}{2\sigma^2} \mid \mid x - \mu_\theta(z)\mid \mid ^2
$$  
...
$$
\text{Maximize } \log p_\theta(x\mid z) \iff \text{Maximize } \left( C - \frac{1}{2\sigma^2} \mid \mid x - \mu_\theta(z)\mid \mid ^2 \right)
$$  

상수 $$C$$와 계수 $$\frac{1}{2\sigma^2}$$를 제외하고 핵심만 보면, 이는 다음 식을 **최소화**하는 것과 수학적으로 완전히 동일. (음수 부호가 있으므로 최대화 $$\rightarrow$$ 최소화로 바뀜)  

$$
\text{Minimize } \mid \mid x - \mu_\theta(z)\mid \mid ^2
$$  

4. MSE와의 연결  
위 식 $$\mid \mid x - \mu_\theta(z)\mid \mid ^2$$는 원본 데이터 $$x$$와 디코더의 예측값 $$\mu_\theta(z)$$(즉, $$\hat{x}$$) 사이의 **유클리드 거리의 제곱(Squared Euclidean Distance)** 임.  
...
$$
\text{Maximize } \log p_\theta(x\mid z) \approx \text{Minimize } \text{MSE}(x, \hat{x})
$$  
...
$$
-D_{KL}(q_\phi(z\mid x) \mid \mid  p_\theta(z)) = \int q_\phi(z\mid x) (\log p_\theta(z) - \log q_\phi(z\mid x)) dz
$$  
...
1. **사전 분포 (Prior) $$p(z)$$:**  
    ...
    - $$p(z) = \mathcal{N}(z; 0, I)$$
2. **근사 사후 분포 (Approximate Posterior) $$q(z\mid x)$$:**  
    - 인코더가 예측한 평균 $$\mu$$와 분산 $$\sigma^2$$를 갖는 정규분포입니다.
    - $$q(z\mid x) = \mathcal{N}(z; \mu, \sigma^2 I)$$
