...
* **비대칭성 (Asymmetric):** $$D_{KL}(P\mid \mid Q) \neq D_{KL}(Q\mid \mid P)$$이므로, 이는 엄밀한 의미의 '거리(Distance)' 개념이 아님.
...
$$
JSD(P\mid \mid Q) = \frac{1}{2}D_{KL}(P\mid \mid M) + \frac{1}{2}D_{KL}(Q\mid \mid M)
$$  

**JSD의 핵심 특징:**  
1.  **대칭성 (Symmetric):** $$P$$와 $$Q$$를 바꿔도 값이 동일 ($$JSD(P\mid \mid Q) = JSD(Q\mid \mid P)$$). 따라서 두 분포 사이의 **'거리' 개념**으로 해석하기 적합.  
