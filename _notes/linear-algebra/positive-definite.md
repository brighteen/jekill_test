...
여기서 full rank라는건 B가 직사각형 행렬이므로 full column rank일 때(B의 열벡터들이 선형 독립)를 의미한다. 명제 4에서 $$\mathbf{x}^\top B^\top B \mathbf{x} = \Vert B\mathbf{x}\Vert ^2 \ge 0$$임을 확인했기 때문에 $$B^\top B$$가 양의 정부호가 되기 위해서는 영이 아닌 벡터 $$\mathbf{x}$$에 대해 $$\Vert B\mathbf{x}\Vert ^2 > 0$$임을 보이면 된다. 앞서 B의 열벡터들이 선형 독립이므로 동차방정식 $$B\mathbf{x} = x_1\mathbf{b}_1 + \cdots + x_n\mathbf{b}_n = \mathbf{0}$$을 만족하는 해는 자명한 해가 유일한 해이다. 따라서 $$\mathbf{x} \ne \mathbf{0}$$이면 $$B\mathbf{x} \ne \mathbf{0}$$이고, 따라서 $$\Vert B\mathbf{x}\Vert ^2 > 0$$이 성립한다.  
</div>
