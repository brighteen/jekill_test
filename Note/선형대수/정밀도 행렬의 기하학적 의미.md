...
정밀도의 관점에서 $1/\lambda_1$은 어떤 '확신의 강도'라고 볼 수 있다. 데이터가 사방으로 흩어져 있어 가장 불확실한 방향이므로, 이 방향에 대해서 우리가 가질 수 있는 확신이 가장 작을 수밖에 없다.
반대로 $\lambda_n$에 대응되는 고유벡터 방향으로 데이터의 분산이 거의 0에 가깝다. 이 방향에 대해 가장 강력하게 확신할 수 있으며, 정밀도의 관점에서 $1/\lambda_n$는 가장 큰 값이 된다.
